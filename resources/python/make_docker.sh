...
DOCKER_IMAGE_NAME="${1:-opinicus_${GIT_HASH}}"

DOCKER_FILE_CHANGES=$(git diff docker_env/Dockerfile)

REQUIREMENT_FILE_CHANGES=$(git diff requirements.txt)

if ! docker inspect --type=image "${DOCKER_IMAGE_NAME}" > /dev/null || [ -n "${DOCKER_FILE_CHANGES}" ] || [ -n "${REQUIREMENT_FILE_CHANGES}" ]; then
...
DOCKER_WORK_DIR="${WORKDIR:-/build/opinicus}"
PREFIX="/usr"
